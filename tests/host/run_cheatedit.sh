...
# hardware fallback: an ADD must shift FOUR parallel arrays in lockstep (records, code
# strings, flag mirror, trainer freeze indices) and a REPLACE must leave a long name and
...
# trainer.h / cheatedit.h / cheatcode.h, so the addresses and offsets under test are the
# shipping ones.
set -u
cd "$(dirname "$0")"
CC="${CC:-cc}"
