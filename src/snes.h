...
#define SNES_CMD_TRAINER_CHEAT       (0x43) /* in-game TRAINER tab: turn the pending request in the trainer meta block ($FA4038, TR_REQ) into a RUNTIME cheat record and redeploy. The payload does NOT ride in MCU_PARAM (12 bytes, the request needs 9): the tab owns the block and $FA is writable in-game through the IS_PATCH identity window, so it fills TR_REQ/TR_REQ_SLOT/TR_REQ_OFF/TR_REQ_VAL and this command carries nothing. Freezing deliberately reuses the ordinary cheat path -- the record lands at $D00000+512*idx and cheat_program() emits it as one of the 20 WRAM patches at $2AD8 -- so a frozen value obeys the master switch, survives CMD_CHEAT_REPROGRAM and shows up in the CHEATS tab for free. ACK = snes_set_mcu_cmd(0). Bounded (a few PSRAM blocks + cheat_program, no SD); lockstep with CMD_TRAINER_CHEAT in snes/memmap.i65 */
