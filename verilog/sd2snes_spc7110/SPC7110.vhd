...
architecture rtl of SPC7110 is
	...
	-- Step 25: tracks whether DEC_BUF contents reflect a currently-owned
	-- decompression session. Set at DS_INIT -> DS_DECODING transition,
	-- cleared on abort (LOAD_RUN / DP_READ). While '0', $4800 and $50:xxxx
	-- reads return x"00" instead of stale post-abort DEC_BUF bytes.
	signal DEC_SESSION_VALID	: std_logic := '0';
	...
	-- Step 24: DEC gets its own /4 clock at 24 MHz.
	-- The MiSTer DEC RTL uses both edges of the clock, so keep the phase
	-- relationship predictable for the surrounding fast-domain FIFO glue.
	signal CLK_DEC				: std_logic := '0';
	signal CLK_DEC_PHASE		: unsigned(1 downto 0) := (others => '0');  -- 0..3 (Step 24: /4)
	-- CDC: FIFO read pulse (one fast cycle per asserted slow cycle)
	signal DEC_IN_RD_AT_P0	: std_logic := '0';
	-- CDC: DEC_OUT_WR -> one-fast-cycle write pulse into DEC_BUF
	signal DEC_OUT_WR_PREV	: std_logic := '0';
	signal DEC_OUT_WR_PULSE	: std_logic;
	-- CDC: DEC_INIT one-fast-cycle pulse -> one-slow-cycle pulse (toggle synchronizer)
	signal DEC_INIT_TOG_FAST	: std_logic := '0';
	signal DEC_INIT_TOG_SYNC	: std_logic_vector(1 downto 0) := (others => '0');
	signal DEC_INIT_SLOW		: std_logic;
	-- CDC: DEC_RUN level -> slow-domain 1-FF synchronizer (Step 19b: halved latency)
	signal DEC_RUN_SYNC		: std_logic := '0';
	signal DEC_RUN_SLOW		: std_logic;
	...
	signal DBG_DO_H : std_logic_vector(3 downto 0);
	signal DBG_DO_L : std_logic_vector(3 downto 0);

begin

	process( RST_N, CLK)
	variable INC : std_logic_vector(23 downto 0);
	-- Step 26: accumulates DEC_BUF_RD_ADDR increments within one CLK cycle.
	-- CPU reads ($4800 / $50:xxxx) and decoder mode-10 writes can all land
	-- on the same cycle. Applying them as separate signal assignments lets
	-- the last-writer win (VHDL rule), silently dropping the others. Using
	-- a variable sum avoids the collision: every trigger adds to the delta,
	-- and one final signal assignment applies the total.
	variable RD_ADDR_DELTA : unsigned(5 downto 0);
	-- Step 26: absolute-reset flag for DEC_BUF_RD_ADDR (set by $4806 write).
	-- Takes priority over the delta path so same-cycle events cannot
	-- re-increment a freshly-reset pointer.

	variable RD_ADDR_RESET : std_logic;
	begin
		if RST_N = '0' then
			...
			if ENABLE = '1' then
				-- Step 26: clear per-cycle delta before any trigger may add to it.
				RD_ADDR_DELTA := (others => '0');
				RD_ADDR_RESET := '0';
				...
				if CA(22) = '0' and CA(15 downto 8) = x"48" and CA(7) = '0' then 
					if CPUWR_N = '0' and SYSCLKF_CE = '1' then	--IO write
						case CA(6 downto 0) is
							...
							when "0000100" =>						--4804
								DECREGS.INDEX <= DI;
								LOAD_RUN <= '1';
								-- A new directory-entry load invalidates the previous
								-- ready indication. Otherwise software that polls $480C
								-- can see stale "done" and DMA $4800 while DS is still
								-- preloading the new stream.
								DEC_DONE <= '0';
							when "0000101" =>						--4805
								DECREGS.OFFSET(7 downto 0) <= DI;
							when "0000110" =>						--4806
								DECREGS.OFFSET(15 downto 8) <= DI;
								-- Step 26: flag absolute reset so the final delta-apply
								-- line below uses 0 instead of DEC_BUF_RD_ADDR as base.
								RD_ADDR_RESET := '1';
								DEC_START <= '1';
								DEC_DONE <= '0';
							when "0000111" =>						--4807
						...
						case CA(6 downto 0) is
							when "0000000" =>						--4800
								-- Step 26: use delta so same-cycle decoder pulse still applies.
								RD_ADDR_DELTA := RD_ADDR_DELTA + 1;
								DECREGS.COUNTER <= std_logic_vector( unsigned(DECREGS.COUNTER) - 1 );
							
							when "0001100" =>						--480C
								-- Hardware clears the ready bit after a successful status
								-- read. Keeping it sticky lets a following decompression
								-- reuse stale readiness and begin DMA before DS_PRELOAD
								-- has produced a valid $4800 stream.
								DEC_DONE <= '0';
					...
					-- Step 26: use delta (see $4800 comment above).
					RD_ADDR_DELTA := RD_ADDR_DELTA + 1;
				end if;
				
				if DEC_OUT_WR_PULSE = '1' then
					if DECREGS.MODE = "10" and DECREGS.OFFSET /= x"0000" then
						-- Step 40: auto-advance rate must match DEC_MODE packing rate.
						-- DEC_MODE=00 fills 1 slot/pulse, DEC_MODE=01 fills 2 slots/pulse,
						-- DEC_MODE=10/11 fill 4 slots/pulse. The original hardcoded +4
						-- assumed DEC_MODE=10 packing and produced RD racing ahead of WR
						-- whenever the game combined DECREGS.MODE=10 streaming with a
						-- smaller DEC_MODE packing (for example SPL4 glitchy team
						-- lineup: DECREGS.MODE=10, DEC_MODE=01, 2 slots/pulse vs +4
						-- RD advance, one full desynchronisation per session).
						case DEC_MODE is
							when "00"   => RD_ADDR_DELTA := RD_ADDR_DELTA + 1;
							when "01"   => RD_ADDR_DELTA := RD_ADDR_DELTA + 2;
							when others => RD_ADDR_DELTA := RD_ADDR_DELTA + 4;
						end case;
						DECREGS.OFFSET <= std_logic_vector( unsigned(DECREGS.OFFSET) - 1 );
					end if;
				end if;
				-- Step 26: apply the accumulated delta once. Wraps naturally at
				-- 64 via unsigned(5 downto 0), matching original behaviour.
				-- RD_ADDR_RESET from $4806 clears the pointer instead.
				if RD_ADDR_RESET = '1' then
					DEC_BUF_RD_ADDR <= (others => '0');
				else
					DEC_BUF_RD_ADDR <= DEC_BUF_RD_ADDR + RD_ADDR_DELTA;
				end if;
	...
	begin
		if RST_N = '0' then
			...
			if CA(22) = '0' and CA(15 downto 8) = x"48" and CA(7) = '0' and SYSCLKR_CE = '1' then
				case CA(6 downto 0) is
					when "0000000" =>						--4800
						-- Step 25: return x"00" between abort and next session
						-- (LOAD_RUN/DP_READ dropped DEC_SESSION_VALID to '0').
						if DEC_SESSION_VALID = '1' then
						  if FIFO_EMPTY = '1' then
							  DO <= x"5a";
							else
								DO <= DEC_BUF_OUT; -- (DBG_DO_H & DBG_DO_L);
							end if;
						else
							DO <= x"55";
						end if;
		...
		if RST_N = '0' then
			...
			if ENABLE = '1' then
				...
				case DS is
					...
					when DS_PRELOAD =>
						-- Step 25: honour mid-preload aborts so a $4804/$4814
						-- that arrives before FIFO_FULL also cleanly returns
						-- DS to IDLE instead of stalling here.
					...
					when DS_INIT =>
						-- Step 25: honour aborts that arrive during the 1-cycle
						-- DS_INIT window; otherwise a stale session could slip
						-- into DS_DECODING before the abort is seen.
						if LOAD_RUN = '1' or DP_READ = '1' then
							DEC_RUN <= '0';
							DEC_INIT <= '0';
							DEC_SESSION_VALID <= '0';
							...
							DEC_RUN <= '1';
							DEC_BUF_WR_ADDR <= (others => '0');
							DEC_SESSION_VALID <= '1';  -- Step 25: DEC_BUF ownership starts now
							DS <= DS_DECODING;
						end if;
	...
	-- Step 24: CLK_DEC generator (/4 divide, 24 MHz, 50% duty).
	-- Replaces Step 18's /6 (16 MHz). 24 MHz is above MiSTer-native 21.47 MHz,
	-- giving the Golomb decoder enough throughput margin to stay ahead of
	-- a SNES DMA (~2.68 MB/s) reading sequentially from $50:xxxx, which was
	-- outrunning the decoder's scatter-fill pattern in mode 10/11 at 16 MHz.
	-- Phase counter 0..3. CLK_DEC rises at phase 0, falls at phase 2.
	CLK_DEC_GEN : process(CLK)
	begin
		if rising_edge(CLK) then
			if CLK_DEC_PHASE = 3 then
				CLK_DEC_PHASE <= (others => '0');
				CLK_DEC <= '1';
			elsif CLK_DEC_PHASE = 1 then
				CLK_DEC_PHASE <= CLK_DEC_PHASE + 1;
				CLK_DEC <= '0';
			else
				CLK_DEC_PHASE <= CLK_DEC_PHASE + 1;
			end if;
	...
	-- Step 18: FIFO read pulse generator.
	-- DEC asserts DEC_IN_RD (= INT_RD) on its falling edge (fast phase 2 of the
	-- prior slow cycle). At fast phase 0 of the current slow cycle DEC latches
	-- DI = FIFO_Q into IN_BUF (we want OLD q here). Then at fast phase 1 we pulse
	-- FIFO_RD so the FIFO advances AFTER DEC has captured. Back-to-back reads
	-- work because the sample-and-pulse repeats each slow cycle DEC_IN_RD is high.
	-- Step 24: sample phase moves 5 -> 3 to match the new /4 phase range.
	FIFO_RD_PULSE_PROC : process(CLK)
	begin
		if rising_edge(CLK) then
			if CLK_DEC_PHASE = 3 then
				DEC_IN_RD_AT_P0 <= DEC_IN_RD;  -- sampled just before slow rising edge
			end if;
		end if;
	end process;
	FIFO_RD <= '1' when (CLK_DEC_PHASE = 1) and (DEC_IN_RD_AT_P0 = '1') else '0';
	FIFO_CLR <= DEC_START;

	-- Step 18: DEC_OUT_WR edge detector -> one-fast-cycle pulse for DEC_BUF writes.
	-- DEC_OUT_WR stays high for one slow cycle (= 6 fast cycles); without this the
	-- fast-domain DEC_BUF writer would fire 6 times per single DEC write.
	DEC_OUT_WR_EDGE : process(CLK)
	begin
		if rising_edge(CLK) then
			DEC_OUT_WR_PREV <= DEC_OUT_WR;
		end if;
	end process;
	DEC_OUT_WR_PULSE <= DEC_OUT_WR and not DEC_OUT_WR_PREV;

	-- Step 18: DEC_INIT CDC (fast one-cycle pulse -> slow one-cycle pulse).
	-- DEC_INIT is driven in the fast-domain FSM as a 1-fast-cycle pulse. A naive
	-- sample into the slow domain would miss it 5/6 of the time. Toggle-sync
	-- guarantees exactly one slow cycle of DEC_INIT_SLOW per fast pulse, regardless
	-- of phase alignment.
	DEC_INIT_TOG_PROC : process(CLK)
	begin
		if rising_edge(CLK) then
			if DEC_INIT = '1' then
				DEC_INIT_TOG_FAST <= not DEC_INIT_TOG_FAST;
			end if;
		end if;
	end process;

	DEC_INIT_SYNC_PROC : process(CLK_DEC)
	begin
		if rising_edge(CLK_DEC) then
			DEC_INIT_TOG_SYNC <= DEC_INIT_TOG_SYNC(0) & DEC_INIT_TOG_FAST;
		end if;
	end process;
	DEC_INIT_SLOW <= DEC_INIT_TOG_SYNC(1) xor DEC_INIT_TOG_SYNC(0);

	-- Step 18: DEC_RUN level synchronizer into slow domain.
	-- Step 19b: reduced to 1-FF to halve halt latency (2 slow cycles -> 1 slow cycle).
	-- Safe because CLK and CLK_DEC share a PLL source (synchronous integer-divide);
	-- edge alignment is deterministic, not truly metastable.
	DEC_RUN_SYNC_PROC : process(CLK_DEC)
	begin
		if rising_edge(CLK_DEC) then
			DEC_RUN_SYNC <= DEC_RUN;
		end if;
	end process;
	-- If PSRAM arbitration starves the decompressor input FIFO, pause the
	-- arithmetic decoder instead of allowing a read from an empty FIFO to
	-- desynchronize the compressed bitstream. The FIFO is refilled by the
	-- fast-domain work state machine while DEC_RUN is held low.
	DEC_RUN_SLOW <= DEC_RUN_SYNC; -- and not FIFO_EMPTY;

	dec : entity work.SPC7110_DEC
	PORT MAP (
		RST_N 	=> RST_N,
		CLK 		=> CLK_DEC,         -- Step 24: CLK_DEC now 24 MHz (was Step 18 /6 16 MHz)
		ENABLE 	=> '1',

		DI 		=> FIFO_Q,
		RD 		=> DEC_IN_RD,

		INIT 		=> DEC_INIT_SLOW,   -- Step 18: was DEC_INIT (fast-domain pulse)
		RUN 		=> DEC_RUN_SLOW,    -- Step 18: was DEC_RUN (fast-domain level)
		MODE 		=> DEC_MODE,
